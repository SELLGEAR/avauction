...
| **BidProwl (MCP aggregator)** | ❌ | REJECTED — surveyed Sept 24, 2026 | Aggregates ~100k live US government / municipal surplus lots from 27 sources (GovDeals, PublicSurplus, HiBid, Proxibid, Municibid, Purple Wave, Wisconsin Surplus, GSA Auctions, PropertyRoom). Read-only survey of 12 priority brands (L-Acoustics, d&b, Meyer Sound, JBL VTX, DiGiCo, Yamaha CL5, Avid S6L, ROE, Absen, Martin MAC, grandMA, Chauvet Maverick): 3 genuine hits total (L-Acoustics Arcs on PublicSurplus, 0 bids), everything else zero or false substring matches ("grandMA" → grandma dishes, "Martin MAC" → MacBooks). **Rejected because:** (1) install/municipal gear only — school Yamaha M7CL/TF5, JBL Control ceiling speakers, ETC Source Four, Chauvet DJ line; no touring-grade inventory; (2) salvage-grade conditions — grades are "parts" / "damaged" / "broken" or blank, titles often generic ("Audio Equipment", "Sound Mixer – Lot of 5"); (3) no AV category — sold comps aggregate only by BidProwl's own categories and there is no audio / lighting / AV bucket (closest is "electronics", ~35k closed lots, median under $50); (4) sold comps are not searchable by keyword — per-listing comps are paywalled behind their Pro tier. Would map to `auction_sold_liquidation` (weight 0.25, floor only) if ever revisited, and would need the three-gate clean-source check against the underlying sites, not just the aggregator. Do not build. |
