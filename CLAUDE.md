...
8. ✅ **FIXED Aug 21, 2026 — "Closest to me" sort.** Root cause was two-layered: (1) the `zip_codes` centroid table was EMPTY — `scripts/seed-zipcodes.ts` had never been run against this project, so `search_listings(sort=nearest)` returned `buyer_zip_required_for_nearest` for every zip; now seeded (33,791 Census ZCTA centroids, script is idempotent). (2) UI wedge: once a zip was stored, re-clicking the chip was a state no-op (no refetch) and the zip prompt could never reopen (`!hasZip` gate), so errors stuck with no retry path. Fixed: clicking "Closest to me" while nearest is active reopens the zip prompt; every zip submission bumps a `refresh` token through `useAuctionLots` forcing a page-1 refetch even with unchanged values; unrecognized-zip errors get a specific message instead of the generic "couldn't load." Verified at the RPC level (37203 → SD12 at 0 mi, correct distance ordering). Original note: entering a zip should clear any prior sort error state and re-run the nearest sort; it stuck (noted Aug 19, 2026).
9. **Auction operations admin dashboard — not built, not needed before launch.** Scope: schedule or skip weeks, assign approved lots to a week's drop, monitor the live Friday auction, pull a lot mid-auction. Needed before *regular weekly operation*, but early auctions can be run manually: "skip a week" simply means not assigning lots to that Monday's drop, and timing settings are adjusted directly in `pricing_engine_settings` (see Auction Format). Build the dashboard once manual operation becomes the bottleneck, not before.
10. **CATALOG MISCATEGORIZATION — now visible in the UI (noted Aug 19, 2026; the Aug 12 known-data-quality issue surfacing).** The grandMA3 Full-Size Console (MA Lighting) shows category "Audio" on its browse card — it's a lighting console. This is the `master_equipment.category` problem already flagged in the Master Equipment Database section ("KNOWN DATA QUALITY ISSUE — flagged Aug 12, 2026"), now reaching buyers through the browse grid. **Investigate next session:** (a) is it only the demo lots' records or systematic across the 268k catalog — query `master_equipment` for the count and pattern (start with known-lighting manufacturers filed as audio: MA Lighting, Martin, Robe, ETC, Avolites); (b) which scraper's category-mapping rule is wrong — is lighting gear landing in Audio generally (AV-iQ contributed 239k records, so its mapping is the prime suspect); (c) fix the shared category-mapping library and re-run/correct affected rows — per the Aug 12 note, flag manufacturer-vs-category mismatches for review rather than blind-overwriting, and handle staging/rigging manufacturers separately since manufacturer→category isn't 1:1 there. Affects browse category filtering and the pricing gauge's category-based comparables — both key off this field. Do this before either ships as trustworthy.
11. ⚠️ **LAUNCH-BLOCKING: Supabase "Confirm email" is OFF (added Aug 21, 2026, auth-slice session).** Turned off in the Supabase dashboard (Authentication → Sign In / Providers → Email → "Confirm email" toggle) so the minimum sign-up → bid loop works without an email round-trip. **Before soft launch: re-enable it and build the confirmation flow.** The `/auth` page already degrades gracefully if it's on (signUp with no session shows "check your email" and flips to sign-in mode), but the full flow — confirmation email template, redirect URL handling, resend — is unbuilt. Do not soft-launch with unconfirmed-email accounts able to bid.

12. **Seller contact identity collected; counterparty reveal NOT built (decided + migrated Aug 22, 2026, migration 0032).** Decision: the full contact set — name, email, phone, physical address — is collected at seller signup for ALL account types (trust signal for a high-end verified marketplace, not friction; browser autofill via proper `autocomplete` attributes keeps it painless). Status map:
    - **(a) Collected and hidden-by-default.** New `sellers` columns (`contact_name`, `contact_email`, `address_line1/2`, `city`, `state`, `postal_code`, `country`) are automatically private: `sellers_select_own` RLS (0006) restricts reads to own-user/admin, and public API routes expose only `anonymous_username`/`verification_status`/`seller_tier`. Nothing further to build for concealment.
    - **(b) Reveal-after-funded-sale is the future piece** — a contact block on a (not-yet-existing) `GET /api/transactions/[id]`, gated on transaction `status ≥ payment_captured` per the anonymity rule ("usernames until escrow funded"). Build it in the post-Korea checkout/transaction UI slice, NOT before.
    - **(c) Buyer shipping address is also uncollected** — checkout stops at SOLD, so the counterparty contact exchange is unbuilt in BOTH directions. The checkout slice must collect the buyer's shipping address and reveal both parties' blocks post-funding.
    - This collection work advances the **INFORM Consumers Act module** (item: "REQUIRED BEFORE LAUNCH"), which requires exactly this seller identity data for high-volume sellers — collection is now done at signup; the INFORM module still needs verification/disclosure logic on top.

13. ✅ **BUILT Aug 23, 2026 — gear entry form (Day 2 of the pre-Korea build).** `/sell/new` multi-step flow: equipment typeahead → 8-question QC checklist → suggested grade (decided A Excellent / B Very Good / C Good / D Fair names) → details → photos stub (photo upload still deferred, `min_photos_per_listing` still 0 — the 0031 launch-blocker to flip to 8 stands) → pricing with honest empty-state suggestion → attestation → submit, handling all three outcomes (success → pending_review; 422 `pending_equipment_review`; validation errors). New endpoints: `GET /api/equipment/search` (typeahead, signed-in + rate-limited) and `GET /api/equipment/[id]/price-suggestion` (range + confidence only, per the moat rule). **Migrations 0033 (`search_equipment` RPC) and 0034 (indexed `match_equipment`) both applied live** via the SQL editor. Verified: 19/19 end-to-end API checks, fuzzy-match harness all-pass, full browser walk. Grade-mapping decision **confirmed by Sean Aug 23**: known issues → D (Fair) per the decided definitions (was C), and flight case no longer affects the suggested condition grade (value add-on, still recorded as `flight_case_included`).

14. ✅ **FIXED Aug 23, 2026 — `match_equipment()` timed out against the 268k catalog; this had silently broken the "can't find it?" path AND would have broken every Phase B scraper.** The 0020 version scored every row exhaustively ("table is small" — stale since seeding); at 268,048 rows it exceeded the statement timeout, so `match_or_queue` 500'd on every call. Migration 0034 rewrites it with an index-assisted candidate prefilter over the 0020 gin index (only pg_trgm's indexable orientations — `column % constant`, `constant <% column`; the naive `column <% constant` form seq-scans). Thresholds set via transaction-local `set_config` in the body because Supabase refuses `CREATE FUNCTION ... SET` on extension GUCs (42501); word-sim prefilter at 0.40, deliberately below the 0.45 queue band. **This unblocks the Phase B scrapers** — they all funnel through `match_or_queue`. ⚠️ **Perf note for Phase B:** submit-time matching now runs 0.8–2.7s per call — fine per seller submission, but watch throughput when scrapers run at volume; if it's the bottleneck, batch calls or tune the prefilter before widening any timeout. Accepted limitations (scraper-only, documented in 0034): alias-only matches with near-zero key overlap, and extremely long dirty titles (>~4-5× key length), can miss the prefilter and fall to queue/reject.

15. **Poor / For Parts: guard shipped Aug 23, 2026 — FULL support still deferred.** `gradeFromQc` now returns `poor` for doesn't-power-on (per the decided scale — Poor is outside A–D and never priced), and submission is blocked in BOTH the form (block panel at the grade step) and a server guard in `POST /api/listings/submit` (422 `poor_for_parts_not_supported`) — the double block protects pricing-data integrity (a broken unit stored as D would poison D-grade comps). **Still unbuilt, a future slice:** actual for-parts listings — schema support for the Poor state, "Sold as-is — no price estimate" listing rendering, gauge suppression, search/browse handling, badge treatment.

16. **Rule established Aug 23, 2026: seller submissions are NEVER silently discarded.** `match_or_queue`'s junk rule (score < 0.45 → `rejected`) exists for scrapers; the submit route now catches that decision — and any match *error* — and queues the product into `pending_master_equipment` (source `seller_submission`, deduped on `(source, normalized_title)`) instead of 500ing or dropping it. Every "can't find it?" entry reaches admin review, honoring the database rule that new products go through admin approval. Admin resolution then uses the existing equipment-queue APIs (`link_existing` / `approve_new` / `reject`).

17. ✅ **BUILT Sept 17, 2026 — Day 3 of the seller-flow build: seller-visible listings + the review loop closed end-to-end.** `/seller/listings` shows the seller's own listings grouped **In review** (`pending_review`) / **Live** (`active`, live auction rows get compact `LotCloseCountdown` + link to `/listing/[id]`) / **Returned from review** (`draft`) / **Ended** (`sold`/`expired`/`delisted`), read directly through the `listings_seller_read_own` RLS path with the browser client (the `seller_id` filter is load-bearing — the public-read-active policy would otherwise mix in other sellers' live rows). **Migration 0035 applied live**: `listings.rejection_reason`, written by `reviewListing` on reject (rejection returns a listing to `draft` — there is NO `rejected` status; since `submit_listing` only creates at `pending_review`, a draft IS a returned listing), cleared on approve, seller-visible via existing RLS + grants. Post-submit success panel and seller home now route to `/seller/listings`. **`scripts/approve-listing.mjs`** is the loose-end-#9 manual stand-in: `list` / `approve <id>` (auction approvals get their slot — default end next Friday noon ET, DST-safe, `--offset-min` stagger) / `reject <id> --reason` — drives the REAL `/api/admin/listings` routes as a service-role-provisioned `ops-admin@avauction-demo.local` whose password rotates every run (no stored secret). Verified: 18/18 harness checks (`scripts/_test_seller_listings.mjs` — submit → pending → approve → live on browse + detail → reject → seller-visible reason → cross-seller isolation) plus a full browser walk; test listings delisted after. **Still unbuilt (new loose ends):** (a) **edit/resubmit of returned drafts** — a returned listing has no path back to review; the "Returned from review" panel shows the reason but offers no action; (b) buy-it-now rows on `/seller/listings` deliberately link nowhere — the detail page renders only the auction experience. **Reclassified Sept 24: in scope for the checkout block (#26).**

18. ⚠️ **PRE-LAUNCH: Supabase security advisor flags `bid_history` as SECURITY DEFINER (CRITICAL — can bypass RLS); full advisor review needed (seen on the dashboard Sept 17, 2026).** Root cause located: the view is created `with (security_invoker = false)` in migration 0008 — **deliberately**. The base `bids` table has no public select policy precisely so `max_bid_encrypted` (the proxy ceiling) and bidder identity can never leak; `bid_history` is the sanctioned public window, exposing only `bid_amount`/`is_proxy_bid`/`created_at` for active/sold listings. The advisor is technically correct that it bypasses RLS — the pre-launch task is to decide: (a) keep-and-document as an accepted, reviewed exception (the naive "fix" — flipping to `security_invoker = true` — would require a public select policy on `bids`, and RLS is row-level not column-level, so that would expose the exact columns the design protects; the real alternative is serving bid history only through the API), or (b) restructure. Either way, **run the FULL security-advisor list before soft launch, not just this finding** — treat it like the 37-check empirical audit that caught the two critical RLS failures pre-0027.

19. **ROADMAP (set Sept 17, 2026).** Two-week sprint: seller-visible listings ✅ (Day 3, #17) → email-confirmation flow (#11) → photo upload (and flip `min_photos_per_listing` to 8, the 0031 launch blocker) → checkout/transaction slice (#12b/c). Then October: design overhaul, staging deploy (decide the dev/prod Supabase split), Reverb sold-data import (the staged NEXT-SESSION TASK #1), gauge review, catalog miscategorization audit (#10), legal/formation kickoff. Launch date tentative — decided once the platform is functionally complete.

20. **PRE-EXTERNAL: scrub the "Co-Authored-By: Claude" trailer from existing git history before showing the repo to anyone external (investors / acquirer / hires).** Needs a history rewrite (git filter-repo) + force-push, and invalidates the commit-hash references in this file (`dacdf34`, `a379fe2`, `0ab6265`, …) — treat it as its own task with a hash-reference sweep, never mid-sprint. The trailer is disabled for all future commits as of Sept 17, 2026 (Claude Code user settings, `attribution.commit = ""`).
...
23. ✅ **BUILT Sept 23, 2026 — seller photo upload (the #19 sprint item; flips the 0031 launch blocker).** `/sell/new` step 5 is now a real uploader (`components/seller/PhotoUploader.tsx`): drag-and-drop / picker / phone camera, per-file validation (JPG/PNG/WEBP/HEIC, ≤10 MB), **signed direct-to-Cloudinary uploads** (no unsigned presets — `POST /api/photos` signs each file with a server-chosen `public_id` = `listings/<seller_id>/<uuid>` and `allowed_formats` locked in; `DELETE /api/photos` destroys a pre-submission upload in the seller's own namespace only), 3-wide upload queue with progress, per-file failure isolation (retry / remove, batch continues), reorder by drag or arrows, "Make cover" (position 0 = cover — the browse card and listing hero already read the first approved photo by position), per-photo shot type with guided auto-suggest. **Watermark is a delivery transformation** (`lib/photos/rules.ts` `WATERMARK_TRANSFORM`, `l_text` "avauction.com" bottom-right): `listing_photos.url` stores the watermarked delivery URL, the original is untouched and addressable via the new `public_id` + `cloudinary_version` columns. **Server trust model** (`lib/photos/cloudinary.ts`): the submit route never trusts a client URL — it verifies Cloudinary's *response* signature (sha1 of public_id + version + secret) per photo, checks the seller namespace, rebuilds the URL, re-indexes positions; then `submit_listing()` (**migration 0036**) enforces `min_photos_per_listing` (**now 8**) and the new `required_photo_types` tunable (`powered_on`, `serial_label`) and rejects duplicate public_ids. **Latent bug fixed alongside:** admin approval never flipped `listing_photos.moderation_status` to `approved`, so the first real listing would have gone live with zero visible photos (public policy, `search_listings()` cover and the detail route all filter on approved) — `reviewListing()` now approves pending/borderline photos with the listing (rejected stay rejected). **Grants unchanged by design:** `listing_photos` client insert/update/delete stay ungranted — a client insert would bypass signature verification; `submit_listing()` (service role) is the only write path. Verified: `scripts/simulate-photos.ts` (46 checks incl. Cloudinary's documented signature vector; the schema checks self-skip until 0036 is applied, the live round-trip self-skips until `CLOUDINARY_*` is set), `audit-security.ts` +6 checks (43 total), submission/admin harnesses unchanged and green. **TO DO BEFORE THIS WORKS:** (a) paste `supabase/migrations/0036_listing_photos_cloudinary.sql` into the SQL editor, then rerun `simulate-photos.ts` and `audit-security.ts`; (b) add `CLOUDINARY_CLOUD_NAME` / `CLOUDINARY_API_KEY` / `CLOUDINARY_API_SECRET` to `.env.local` and the Vercel project (see `.env.example`); (c) optional: enable the AWS Rekognition add-on in the Cloudinary console and set `CLOUDINARY_MODERATION=aws_rek` — the plumbing is in place (signed `moderation` param; a rejected upload shows the CLAUDE.md error and is destroyed). **Deliberately not built:** editing photos on an existing/returned listing (the form is create-only; would need a seller-scoped update path), sweeping orphaned uploads from abandoned sessions (namespaced per seller under `listings/<seller_id>/`, so a periodic Admin-API sweep of assets with no `listing_photos` row is straightforward later), moderation status read-back via the Admin API (admin review is the backstop today). `scripts/_test_gear_entry.mjs` (untracked one-off) still submits `photos: []` and will get `min_photos_required` after 0036 — expected.

---

24. ✅ **BUILT Sept 24, 2026 — seller anonymity layer for listing photos (builds on #23).** Two parts. **(1) Delivery lock-down.** #23 uploaded as Cloudinary type `upload`, which meant anyone could strip the watermark transformation from a stored URL and fetch the clean original — a direct anonymity hole (rental-house stencils, asset tags). Every listing photo is now uploaded with `type=authenticated` (locked into the signed upload params, so the browser can't opt out): neither the original nor ANY derived version is addressable without a server-side delivery signature over the exact transformation (`lib/photos/cloudinary.ts` `signedDeliveryUrl`: `s--base64(sha1(transform/public_id.format + secret))[0..8]--`, version excluded, per the Cloudinary SDK). `listing_photos.url` stores the signed buyer URL (blur + watermark); the harness proves live that the stripped, edited, unsigned and `/image/upload/` variants are all rejected while the buyer URL and the admin-signed original resolve. Clean originals are signed ONLY in `getListingPhotosForReview()` (admin routes) and the server-side detection fetch. Previews for the uploader now come from `POST /api/photos/preview` (signed thumb + clean editor URL, owner namespace only) since the upload response's `secure_url` is no longer viewable. **Console setting to change by hand: Settings → Security → enable "Strict transformations".** Not strictly required for `authenticated` assets (signed URLs are already mandatory there), but it closes the same hole for any `upload`-type asset that ever lands in the account, and signed URLs pass through it unchanged. Nothing else in the console needs touching; the account's signature algorithm is SHA-1 (both upload and delivery signatures verified live). **(2) Identifying-mark blur.** After each upload, `POST /api/photos/analyze` fetches a downscaled clean copy server-side and runs Claude vision (`lib/photos/detect.ts`, model from `PHOTO_DETECTION_MODEL`, default `claude-opus-5-5`; structured output, server-side refusal fallbacks enabled) for seller-identifying marks — company names, non-manufacturer logos, asset tags, stencils, spray paint, contact info — and explicitly NOT manufacturer branding, model numbers or serial plates; the serial-label shot gets an extra "keep the plate legible" instruction. Suggestions land on the tile as boxes; `components/seller/BlurEditor.tsx` lets the seller move/resize/add/delete before submitting. **Made unmissable Sept 24 (second pass):** a prominent notice tops the photos step (blur company names, logos, asset tags, stencils, spray paint, stickers, handwriting; manufacturer logos and serial plates stay visible; why: anonymity until escrow), tiles with unreviewed suggested boxes carry a "Review blur" badge until the seller opens them, and Continue is gated on a checkbox — "I've checked every photo and blurred all company names, logos, and asset tags." — which resets whenever the photo set changes. `photoStepGate()` in `lib/photos/rules.ts` is the pure rule; `POST /api/listings/submit` re-checks `blur_attested` server-side (400 `blur_attestation_required`) so a client that skips the checkbox can't submit photos. **Fail-open by design:** no key, timeout, error or refusal → status `failed`/`unavailable`, no suggestions, upload never blocked, seller draws boxes by hand. **Migration 0037:** `listing_photos.blur_regions` (normalized 0..1 rectangles) + `detection` (status, suggested, model, note); `submit_listing()` stores both. Regions are applied on DELIVERY as chained `e_blur_region` components (pixel coords from the stored width/height) ahead of the watermark — the original is never modified. `/admin/listings/[id]` (new, admin-only page) shows the clean original with the boxes drawn beside the buyer view. **Env:** `ANTHROPIC_API_KEY` (in `.env.local`; add to Vercel), optional `PHOTO_DETECTION_MODEL`. **Strict transformations enabled in the Cloudinary console Sept 24** — the live lock-down proof passes with it on. **Rules conflicts noted:** CLAUDE.md's tech-stack line still names `claude-sonnet-4-20250514` — superseded by `claude-opus-5-5` here (the Models API confirms it exists); the six locally-seeded demo lots (`public/demo/*.jpg`, `_seed_demo_photos.mjs`) are public, unwatermarked and outside this lock-down — replace them before soft launch; `listing_photos` rows readable by anon carry `public_id`/`cloudinary_version`/`blur_regions`, which is harmless (nothing is fetchable without a signature) but worth knowing. Verified: `scripts/simulate-photos.ts` (all checks green incl. the live lock-down proof with strict transformations on, a live vision scan, and the 0037 schema checks), `audit-security.ts` 43, submission + admin harnesses green, tsc + build clean.

25. ✅ **BUILT Sept 24, 2026 — seller-visible listings, second pass (extends #17).** What #17 already had: `/seller/listings` grouped by state via RLS, countdowns on live auctions, the rejection reason on returned drafts. What was missing and is now built: **cover photos** (the signed buyer URL from #24 — blur + watermark), **watcher counts** on live rows gated by `min_watchers_to_display` exactly like the public page, **status filter chips**, a **seller detail view** at `/seller/listings/[id]` (every submitted field, the QC checklist, the photos exactly as buyers see them with per-photo blur counts, the seller's own reserve, high bid + bid count + time remaining on live auctions, a "what you can do next" panel on returned drafts), and **withdraw**. Data now comes from a service-role layer scoped by seller_id (`lib/seller/listings.ts` behind `GET /api/seller/listings`, `GET /api/seller/listings/[id]`, `POST /api/seller/listings/[id]/withdraw`; `lib/sellerAuth.ts` `requireSeller`) instead of the browser RLS read — the client can't read `pricing_engine_settings` (threshold) or count `watchlists`, and the payload shape is where the identity walls are enforced. **State machine as the seller sees it** (CLAUDE.md governs; the request's wording mapped onto the real `listings.status` values): pending review = `pending_review`; live = `active` (auction OR buy-it-now — a reserve-not-met auction is auto-bumped to a live buy-it-now by `close_auction_lot()`, so it shows as Live, not unsold); rejected = `draft` + `rejection_reason` (**there is no `rejected` status**); sold = `sold`; unsold/ended = `expired` (only an auction with no asking price to bump to); withdrawn = `delisted` + `delist_reason`. **Withdraw rule** (`withdrawBlock()`): allowed from in-review, returned, live buy-it-now, and a live auction with **no bids**; refused for a live auction with bids (`auction_has_bids` — bidders hold commitments; pulling a lot mid-auction stays an admin operation per #9), sold, expired, delisted. The update carries the status (and `bid_count = 0` for auctions) in its WHERE clause so a race with an incoming bid or a close refuses safely. **Delist prompt** per Non-circumvention: "Was this sold through AVauction.com?" — `sold_privately` and `no_answer` (skipping) log a `seller_strikes` row of `private_sale_flag` with `is_strike = false`; `sold_on_platform` / `no_longer_selling` do not. **Identity walls verified both ways** (`scripts/simulate-seller-listings.ts`, 76 checks; `audit-security.ts` +3 = 46): the seller cannot read `bids` on their own listing (bidder-own policy), `bid_history` carries no bidder id, the seller payloads contain no bidder fields and no admin-only data (quality score, price suggestion); buyers/anon cannot read `sellers`, cannot embed it through `listings`, and the public detail route exposes only the anonymous block. A withdrawn listing leaves public search and the public route (404) but stays in the seller's list. **Still out of scope:** edit/resubmit of returned drafts (#17a) — the returned panel says so and points to "withdraw, then list again"; buy-it-now public page (#17b) — seller detail links to the public page only for auctions; **now in scope for the checkout block (#26), not deferred**. No migration: `delist_reason`, `seller_strikes.private_sale_flag` and the seller RLS policies all existed since 0007/0010. `scripts/_test_seller_listings.mjs` (untracked one-off from #17) is superseded by the new harness.

26. **DECIDED Sept 24, 2026 — the buy-it-now public page is IN SCOPE for the checkout block, not deferred.** Supersedes the "later" wording in #4, #17b and #25. Checkout has **two entry points** — (1) **auction win**: a lot closes at or above reserve, `close_auction_lot()` creates the `pending_payment` transaction, the winner pays; (2) **buy-it-now**: a buyer clicks buy on a live buy-it-now listing (including auctions auto-bumped after a reserve-not-met close), `POST /api/listings/[id]/purchase` / the buy-now RPC creates the transaction, the buyer pays. The buy-it-now page is the front door to entry point (2), so it ships with checkout, not after it. **Night 5 mapping (the checkout block's route/state/transaction mapping) must cover BOTH entry points**: public `/listing/[id]` rendering a buy-it-now listing with its own accent and a buy action; the SOLD state for both paths; the buyer shipping address (#12c) and the post-funding counterparty reveal (#12b) for both; the seller detail view linking to the public page for buy-it-now listings too (today it links only for auctions). Roadmap #19's "checkout/transaction slice (#12b/c)" is this block.

27. ⚠️ **LAUNCH-BLOCKING (logged Sept 24, 2026, not yet built): the condition checklist and the required photo types are the same for every item, so non-powered gear cannot be listed at all.** The gear entry form asks every seller the same 8 QC questions ("Powers on and produces full output?", "Flight case or road case included?", "Serial number confirmed?" …) and `submit_listing()` requires a `powered_on` and a `serial_label` photo for every listing (0036 `required_photo_types`). Road cases, truss, rigging hardware, cable, staging decks and risers can't truthfully answer those questions and can't produce those shots — and "doesn't power on" routes to the Poor/For-Parts block (#15), so a road case is unlistable by construction. **Fix (do not build until scoped): category-aware checklists and photo requirements, keyed off the catalog category** (`master_equipment.category`, which every listing inherits). Concretely: (a) a per-category QC question set — `gradeFromQc` and `qc_responses` (0011: `powers_on`, `all_components`, `flight_case`, `cosmetic_damage`, `known_issues`, `serviced`, `serial_confirmed` are all NOT NULL booleans/text) need a "not applicable" answer or per-category columns; (b) `required_photo_types` becomes per-category (a JSON map in `pricing_engine_settings`, or a `photo_requirements` table) and `submit_listing()` reads the listing's category to pick the rule; (c) `lib/photos/rules.ts` `REQUIRED_PHOTO_TYPES` / the uploader's guided shot list follow the same map; (d) the Poor guard (#15) must not fire for gear that has no power state; (e) the quality score's photo-type bonuses (`lib/listings/qualityScore.ts`) should reward the category's own key shots. **Catalog categories that exist today** (`master_equipment.category` check constraint, 0001; live counts Sept 24, 2026, total 268,048 records): `led_video` 33,109 · `audio` 45,852 · `lighting` 21,106 · `staging` 1,603 · `rigging` 788 · `other` 165,590. There is **no cable, case, or accessory category** — road cases, cable and hardware land in `other` (and sometimes in the category of the gear they belong to), and the scrapers' keyword mapper (`scripts/lib/mapCategory.ts`) only knows the six values. Mapping proposal to start from: powered (full checklist, `powered_on` + `serial_label` required) = `led_video`, `audio`, `lighting`; non-powered (no power/serial questions, require `front` + `back` + `damage_closeup` or similar, ask about load rating / certification for `rigging`) = `staging`, `rigging`; mixed = `other`, which needs either a finer sub-category or a per-product "has power" flag before it can be mapped. **Preferred approach (decided Sept 24, 2026):** add a FIRST checklist question — **"Does this unit require power?"** — pre-filled from the catalog category where that's reliable (`led_video`, `audio`, `lighting` → yes; `staging`, `rigging` → no; `other` → unset, seller answers), and always changeable by the seller. Answering **No** skips the power and serial questions ("Powers on", "Serial confirmed"; "Case included" can stay optional) and swaps the required photo list from `powered_on` + `serial_label` to the non-powered set. Because the seller's answer, not the category, drives the rules, this **does not block on the #10 catalog cleanup** — the category only pre-fills. Store the answer on the listing (`requires_power boolean`) so `submit_listing()` can pick the photo rule from it, and make the Poor guard (#15) conditional on it. **Interaction with #10** (catalog miscategorization): remains relevant only for the pre-fill; a wrong category now costs the seller one tap, not a blocked listing.
